<!-- BEGIN MODAL -->
<div x-show="purchaseReturnModal"
    ...
    <!-- Modal Content -->
    <div @click.outside="$refs.purchaseReturnForm.reset(); purchaseReturnModal = false"
         class="relative w-full max-w-lg rounded-3xl bg-white p-6 shadow-lg dark:bg-gray-900">

        <!-- Close Button -->
        <button type="button"
                @click="$refs.purchaseReturnForm.reset(); purchaseReturnModal = false"
        ...
        <h4 class="mb-4 text-2xl font-semibold text-gray-800 dark:text-white">
            Purchase cancellation and Recovery Payment Entry
        </h4>

        <!-- Form -->
        <form x-ref="purchaseReturnForm"
              method="POST"
              action="{{ route('land-purchase.cancel', [$landPurchase->id]) }}"
